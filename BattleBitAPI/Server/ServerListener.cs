        ...
        try
        {
            using (var source = new CancellationTokenSource(Const.HailConnectTimeout))
            {
                using (var readStream = Stream.Get())
                {
                    ...
                    {
                        readStream.Reset();
                        if (!await networkStream.TryRead(readStream, GameServer<TPlayer>.mRoundSettings.Size,
                                source.Token))
                    ...
                    while (clientCount > 0)
                    {
                        ...
                        var player =
                            mInstanceDatabase.GetPlayerInstance(steamid, out var isNewClient, out var playerInternal);
